...
BEGIN  
 ...
 IF @empresa = '01'
 BEGIN
	 SELECT TOP 1 @estped = ESTADO  FROM PEDIDOS01.DBO.PED009  WHERE OPERACION= @numop;
 END
 ELSE
 BEGIN
     SELECT TOP 1 @estped = ESTADO  FROM PEDIDOS02.DBO.PED009  WHERE OPERACION= @numop;
 END;
...
AS      
 ...
 DECLARE     
  ...
  END    
 /*== FIN PAGINACIÓN ===*/   
 SELECT @EMPRESA = EMPRESA_DEFECTO FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS LIKE @ID_USUARIO;
BEGIN
	IF @EMPRESA = '01'
	BEGIN
		WITH TBL_PEDIDOS    
		...
		SELECT     
				...
				P.TOTAL AS IMP_TTOT,    
				PEDIDOS01.DBO.ESTADO_PEDIDO(@EMPRESA, P.OPERACION) AS SWT_PED,
				P.ORDEN_COMPRA,
				P.FACTURA,
				P.OBSERVACIONES
		FROM PEDIDOS01.DBO.PED009 P JOIN PEDIDOS01.DBO.CUE001 C ON P.RUC = C.RUC    
	    JOIN [PEDIDOS00].DBO.GLO002 T    
	...
	IF @EMPRESA = '02'
		...
		SELECT     
				...
				P.FACTURA,
				P.OBSERVACIONES
		FROM PEDIDOS02.DBO.PED009 P JOIN PEDIDOS02.DBO.CUE001 C ON P.RUC = C.RUC    
	    JOIN [PEDIDOS00].DBO.GLO002 T    
		ON P.MONEDA = T.CODIGO WHERE rTRIM(P.USUARIO) = @ID_USUARIO    
		AND (
			STR(TRY_CONVERT(BIGINT, @BUSQUEDA)) LIKE LTRIM(STR(TRY_CONVERT(BIGINT, P.OPERACION))) OR
...
BEGIN
	...
	IF @EMPRESA = '01'
		...
		SELECT
				...
				P.FACTURA,
				P.OBSERVACIONES
			FROM PEDIDOS01.DBO.PED009 P    
				JOIN PEDIDOS01.DBO.CUE001 C    
					ON P.RUC = C.RUC
				JOIN PEDIDOS00.dbo.SUP001 V
					ON V.ALIAS LIKE @ID_USUARIO
				LEFT JOIN PEDIDOS01.dbo.CUE005 U
					ON C.UBIGEO = U.UBIGEO
				LEFT JOIN PEDIDOS01.dbo.CUE017 CON
					ON C.CONDICIONES = CON.CONDICION
		JOIN [PEDIDOS00].dbo.GLO002 T    
	...
	IF @EMPRESA = '02'
		...
		SELECT     
				...
				P.FACTURA,
				P.OBSERVACIONES
			FROM PEDIDOS02.DBO.PED009 P    
				JOIN PEDIDOS02.DBO.CUE001 C    
					ON P.RUC = C.RUC
				JOIN PEDIDOS00.dbo.SUP001 V
					ON V.ALIAS LIKE @ID_USUARIO
				LEFT JOIN PEDIDOS02.dbo.CUE005 U
					ON C.UBIGEO = U.UBIGEO
				LEFT JOIN PEDIDOS02.dbo.CUE017 CON
					ON C.CONDICIONES = CON.CONDICION
		JOIN [PEDIDOS00].dbo.GLO002 T    
		ON P.MONEDA = T.CODIGO
		WHERE rTRIM(P.USUARIO) = @ID_USUARIO AND P.OPERACION = @OPERACION) SELECT TOP(1) * FROM TBL_PEDIDOS; 
...
--SET @USUARIO = 'VENTA01';
	DECLARE @EMPRESA VARCHAR(2);
	SELECT @EMPRESA = EMPRESA_DEFECTO, @IDVENDEDOR = VENDEDOR FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS LIKE @USUARIO;
	BEGIN
		IF @EMPRESA = '01'
		BEGIN
			SET @VALIDO = (SELECT 1 FROM PEDIDOS01.DBO.PED009 WHERE USUARIO = @USUARIO AND OPERACION = @OPERACION)
			SELECT P.IDPRODUCTO, PD.CODIGO_BARRA, CANTIDAD, P.PRECIO, P.IMPUESTO, P.MONTO, P.BASE, 'UND' AS TIPO, A.DESCRIPCION AS ALMACEN, A.IDALMACEN AS COD_ALMACEN, P.DESCRIPCION FROM PEDIDOS01.DBO.PED008 P
				JOIN PEDIDOS01.DBO.INV005 A ON A.IDALMACEN = P.IDALMACEN JOIN PEDIDOS01.dbo.INV001 PD ON PD.IDPRODUCTO = P.IDPRODUCTO
			WHERE OPERACION = @OPERACION AND @VALIDO = 1;
		END;
		IF @EMPRESA = '02'
		BEGIN
			SET @VALIDO = (SELECT 1 FROM PEDIDOS02.DBO.PED009 WHERE USUARIO = @USUARIO AND OPERACION = @OPERACION)
			SELECT P.IDPRODUCTO, PD.CODIGO_BARRA, CANTIDAD, P.PRECIO, P.IMPUESTO, P.MONTO, P.BASE, 'UND' AS TIPO, A.DESCRIPCION AS ALMACEN, A.IDALMACEN AS COD_ALMACEN, P.DESCRIPCION FROM PEDIDOS02.DBO.PED008 P
				JOIN PEDIDOS02.DBO.INV005 A ON A.IDALMACEN = P.IDALMACEN JOIN PEDIDOS02.dbo.INV001 PD ON PD.IDPRODUCTO = P.IDPRODUCTO
			WHERE OPERACION = @OPERACION AND @VALIDO = 1;
		END;
	END;
GO;
...
AS        
 ...
 DECLARE       
  ...
  SET @IMENOS1 = @CANT_FILAS - 1      
  DECLARE @EMPRESA VARCHAR(2);
  SELECT @EMPRESA = EMPRESA_DEFECTO FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS LIKE @USUARIO;
  IF @ALL_REG = 0      
  BEGIN      
   SET @INICIO = ((@NUM_PAGINA * @CANT_FILAS) - @IMENOS1)      
   SET @FIN = (@NUM_PAGINA * @CANT_FILAS)      
 ...
 /*== FIN PAGINACIÓN ===*/      
BEGIN
	SELECT @IGV = IMPUESTO FROM PEDIDOS00.DBO.SUP003 WHERE EMPRESA = @EMPRESA;
	IF @EMPRESA = '01'
	BEGIN
		WITH TBL_STOCK      
		AS (
			SELECT    
				...
				PRECIO4,
				PRECIO5
			FROM PEDIDOS01.DBO.INV001 P    
			JOIN PEDIDOS01.DBO.INV006 S ON P.IDPRODUCTO = S.IDPRODUCTO    
			JOIN PEDIDOS01.DBO.INV005 A ON S.IDALMACEN = A.IDALMACEN     
			AND P.ACTIVO = 1     
			WHERE     
			...
			) AND PEDIDOS01.DBO.VERIFICAR_CLASE(CLASE, @CLASES) = 1   
			GROUP BY     
			   P.IDPRODUCTO,    
			   P.DESCRIPCION,
			   P.USA_IMPUESTO,
	...
	BEGIN
		...
		AS (
			SELECT    
				...
				PRECIO4,
				PRECIO5
			FROM PEDIDOS02.DBO.INV001 P    
			JOIN PEDIDOS02.DBO.INV006 S ON P.IDPRODUCTO = S.IDPRODUCTO    
			JOIN PEDIDOS02.DBO.INV005 A ON S.IDALMACEN = A.IDALMACEN     
			AND P.ACTIVO = 1     
			WHERE     
			(ISNULL(@ID_ALMACEN, 0) = 0 OR A.IDALMACEN = @ID_ALMACEN) AND(    
			CONVERT(VARCHAR(100), P.IDPRODUCTO) LIKE '%'+@PRODUCTO+'%'--COD PRODUCTO    
...
SELECT @EMPRESA = EMPRESA_DEFECTO, @IDVENDEDOR = VENDEDOR FROM PEDIDOS00.dbo.SUP001 WHERE ALIAS = @USUARIO
	IF @EMPRESA = '01'
	BEGIN
		SELECT RUC, CONCAT(RUC, ' | ', RAZON) AS DESCRIPCION, IDVENDEDOR, PRECIO FROM PEDIDOS01.DBO.CUE001 WHERE IDVENDEDOR = @IDVENDEDOR AND ACTIVO = 1 AND CLASE LIKE 'C' AND (RAZON LIKE '%'+ @CRITERIO + '%' OR RUC LIKE '%'+ @CRITERIO + '%');
	END;
	IF @EMPRESA = '02'
	BEGIN
		SELECT RUC, CONCAT(RUC, ' | ', RAZON) AS DESCRIPCION, IDVENDEDOR, PRECIO FROM PEDIDOS02.DBO.CUE001 WHERE IDVENDEDOR = @IDVENDEDOR AND ACTIVO = 1 AND CLASE LIKE 'C' AND (RAZON LIKE '%'+ @CRITERIO + '%' OR RUC LIKE '%'+ @CRITERIO + '%');
	END;
END;
GO;
...
CREATE OR ALTER PROCEDURE USP_SESION_MONEDAS
	AS SELECT CODIGO AS NUM_ITEM, DESCRIPCION AS DES_ITEM, SIMBOLO AS ABR_ITEM FROM PEDIDOS00.DBO.GLO002; 
GO;
...
BEGIN  
DECLARE @EMPRESA VARCHAR(2);
SELECT @EMPRESA = EMPRESA_DEFECTO FROM PEDIDOS00.dbo.SUP001 WHERE ALIAS = @USUARIO
	IF @EMPRESA = '01'
	BEGIN
		SELECT ROW_NUMBER() OVER (ORDER BY TIPO ASC) AS ID, TIPO, DESCRIPCION FROM PEDIDOS01.DBO.CUE008;
	END;
	IF @EMPRESA = '02'
	BEGIN
		SELECT ROW_NUMBER() OVER (ORDER BY TIPO ASC) AS ID, TIPO, DESCRIPCION FROM PEDIDOS02.DBO.CUE008;
	END;
END;
GO;

--- CREAR CLIENTE PARA EL VENDEDOR ACTUAL
...
BEGIN
	...
	SELECT @EMPRESA = EMPRESA_DEFECTO, @VENDEDOR = VENDEDOR FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS = @USUARIO;
	IF @EMPRESA = '01'
	BEGIN
	    IF EXISTS (SELECT 1 FROM PEDIDOS01.dbo.CUE001 WHERE RUC = @RUC AND CLASE LIKE 'C')
		BEGIN
            RAISERROR('El cliente yá se encuentra registrado.', 16, 1);
		END
		ELSE
		BEGIN
			INSERT INTO PEDIDOS01.dbo.CUE001(IDVENDEDOR, RAZON, RUC, DIRECCION, TELEFONO, CIUDAD, CONTACTO, TELEFONO_CONTACTO, CORREO, UBIGEO, CLASE, PRECIO, CONDICIONES, ACTIVO, CREADO_WEB, DOMICILIADO, TIPO_DOCUMENTO, FECHA)
	...
	IF @EMPRESA = '02'
	BEGIN
	    IF EXISTS (SELECT 1 FROM PEDIDOS02.dbo.CUE001 WHERE RUC = @RUC AND CLASE LIKE 'C')
		BEGIN
            RAISERROR('El cliente yá se encuentra registrado.', 16, 1);
		END
		ELSE
		BEGIN
			INSERT INTO PEDIDOS02.dbo.CUE001(IDVENDEDOR, RAZON, RUC, DIRECCION, TELEFONO, CIUDAD, CONTACTO, TELEFONO_CONTACTO, CORREO, UBIGEO, CLASE, PRECIO, CONDICIONES, ACTIVO, CREADO_WEB, DOMICILIADO, TIPO_DOCUMENTO, FECHA)
				VALUES(@VENDEDOR, @RAZON, @RUC, @DIRECCION, @TELEFONO, @CIUDAD, @CONTACTO, @TELEFONO_CONTACTO, @CORREO, @UBIGEO, 'C','1', @CONDICION, 1, 1, 1, @TIPO_DOCUMENTO, GETDATE());
		END;
	END;
END;
...
--- OBTENER LISTA DE CONDICIONES DE LA EMPRESA DEL USUARIO ACTUAL
...
BEGIN
	...
	IF @EMPRESA = '01'
	BEGIN
		SELECT * FROM PEDIDOS01.dbo.CUE017;
	END;
	IF @EMPRESA = '02'
	BEGIN
		SELECT * FROM PEDIDOS02.dbo.CUE017;
	END;
END;
GO;


--- FILTRAR UBIGEOS POR USUARIO
...
BEGIN
	...
	IF @EMPRESA = '01'
	BEGIN
		SELECT TOP(500) UBIGEO, DISTRITO, PROVINCIA, DEPARTAMENTO FROM PEDIDOS01.dbo.CUE005 WHERE DISTRITO LIKE '%'+@BUSQUEDA+'%' OR
			PROVINCIA LIKE '%' + @BUSQUEDA + '%' OR DEPARTAMENTO LIKE '%' + @BUSQUEDA + '%' ORDER BY DEPARTAMENTO;
	END;
	IF @EMPRESA = '02'
	BEGIN
		SELECT TOP(500) UBIGEO, DISTRITO, PROVINCIA, DEPARTAMENTO FROM PEDIDOS02.dbo.CUE005 WHERE DISTRITO LIKE '%'+@BUSQUEDA+'%' OR
			PROVINCIA LIKE '%' + @BUSQUEDA + '%' OR DEPARTAMENTO LIKE '%' + @BUSQUEDA + '%' ORDER BY DEPARTAMENTO;
	END;
END;
GO;
...
AS      
BEGIN      
 SELECT IDREGISTRO, VENDEDOR, NOMBRE, EMPRESAS, EMPRESA_DEFECTO, PUEDE_CAMBIAR_PRECIO_FACTURACION, OPERACIONES_ESPECIALES, PRECIO_PERMITIDOS FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS=@USER;
END
GO;
...
BEGIN
	DECLARE @EMPRESA VARCHAR(2);
	SELECT @EMPRESA = EMPRESA_DEFECTO FROM PEDIDOS00.DBO.SUP001 WHERE ALIAS = @USUARIO;
	SELECT E.EMPRESA, E.RUC, E.CODIGO, ED.PRECIO_TIENE_IMPUESTO, ED.NOMBRE_PRECIO1, ED.NOMBRE_PRECIO2, ED.NOMBRE_PRECIO3, ED.NOMBRE_PRECIO4, ED.NOMBRE_PRECIO5 FROM PEDIDOS00.dbo.SUP002 E JOIN PEDIDOS00.dbo.SUP003 ED ON E.CODIGO = ED.EMPRESA WHERE E.CODIGO = @EMPRESA;
END;
GO;