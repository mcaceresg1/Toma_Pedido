-- =============================================
-- STORED PROCEDURE: USP_GET_REPORTE_PROVEEDOR (CORREGIDO)
...
--         GLO007 (BD de login)
-- FECHA MODIFICACIÓN: 13/12/2025 20:55:00 - Sistema
-- =============================================
-- 
-- Descripción: Obtiene un reporte completo de proveedores
...
--       Filtra por CUE001.CLASE='P' (proveedores)
-- =============================================
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[USP_GET_REPORTE_PROVEEDOR]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [dbo].[USP_GET_REPORTE_PROVEEDOR];
GO

CREATE PROCEDURE [dbo].[USP_GET_REPORTE_PROVEEDOR]
    @BD_LOGIN VARCHAR(50)
-- =============================================
-- STORED PROCEDURE: USP_GET_REPORTE_PROVEEDOR (CORREGIDO)
-- PROYECTO: Toma Pedido
...
BEGIN
    ...
    WHERE CUE001.CLASE = ''P''';
    
    EXEC sp_executesql @Sql;
END;
GO

PRINT 'Stored procedure USP_GET_REPORTE_PROVEEDOR creado exitosamente.';
GO
